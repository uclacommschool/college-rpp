...
Quarto version of 03-build-teacher-report.R -- same report, same UCLA
Center for Community Schooling theme, same single-dropdown-syncs-everything
behavior. This exists as an easier file to open and tweak by hand (edit the
prose/labels/colors right here, then just re-render) instead of editing an
R script that assembles a big HTML string.

Like 03-build-teacher-report.R, this does NOT read or write any intermediate
CSVs -- it runs 01-calculate-outcomes.R and 02-sankey-pathways.R in full
(each into its own R environment, to avoid their same-named variables
colliding -- see the code comments below) and builds the report straight
from their in-memory results. 01 and 02 are left completely unmodified and
still produce all of their own usual outputs.

HOW TO RENDER: open a terminal in this file's folder (same folder as
01-calculate-outcomes.R and 02-sankey-pathways.R) and run:
    quarto render 04-teacher-report.qmd
This produces college_going_outcomes_report.html (self-contained, share the
link from Box/Drive or attach it directly).

Font note: the reference page (communityschooling.gseis.ucla.edu) could only
be read as page text, not live CSS, so its exact web font isn't confirmed --
Roboto / Roboto Slab were chosen as a safe, legible, university-appropriate
substitute. Swap the Google Fonts link + font-family values in the CSS block
below if you confirm the Center's site actually uses something else. Colors
ARE verified against UCLA's official brand guidelines (brand.ucla.edu).
-->
...
# files live in the same folder as this .qmd.
outcomes_script_path <- "01-calculate-outcomes.R" # UPDATE if these live elsewhere
sankey_script_path   <- "02-sankey-pathways.R"     # UPDATE if these live elsewhere

REPORT_TITLE <- "UCLA Community School: College-Going Outcomes"

# Used only to flag KPI tiles for cohorts whose outcome window hasn't fully
# elapsed as of this snapshot (e.g. a Class of 2024 student can't show a
# real 6-year completion figure in a 2025 snapshot). Purely a display note
...
# Sourced into SEPARATE environments (rather than straight into this
# document) so 01 and 02's internal variables never collide -- 01 uses
# `cohort_n` for a per-graduating-year N table, while 02 reuses that exact
# same name `cohort_n` for a single overall-N scalar. Separate environments
# make that collision structurally impossible, and let both scripts run
# exactly as written/tested standalone.
#
# NOTE: each script re-reads and re-cleans the PSD from scratch, so this
# does duplicate that work (a few extra seconds per render) -- kept this way
...
links_general_overall <- env_sankey$links_general
links_general_by_year <- env_sankey$links_general_by_year
links_detail_overall  <- env_sankey$links_detail
links_detail_by_year  <- env_sankey$links_detail_by_year

# Reuse 02's node-order, node-COLOR, and trace-building logic directly (the
# same functions its own standalone Sankey HTML files use) -- per team
...
# "All Cohorts" KPI = a TRUE weighted rate (sum of numerators / sum of
# denominators across every graduating class) -- NOT an average of each
# cohort's rate, which would over-weight small early cohorts against large
# recent ones.
...
per_cohort_kpi <- outcome_summary %>%
  transmute(
    ...
    N,
    Postsecondary_Plans,
    Immediate_Enrollment = Immediate_Enrollment_TermBased,
...
```{css}
/* UCLA Center for Community Schooling theme -- see the palette note at the
   top of this file for the color/font sourcing details. */
...
main.report-main { max-width:1080px; margin:0 auto; padding:1.75rem 1.5rem 3rem; }
...
footer.report-footer { font-size:.78rem; color:var(--ink-soft); line-height:1.65; border-top:1px solid var(--border);
  max-width:1080px; margin:1.75rem auto 0; padding:1.25rem 1.5rem 0; }
footer.report-footer p { margin:0 0 .6rem; }
footer.report-footer strong { color:var(--ink); }
...
# Everything below is assembled as ONE string and returned via
# htmltools::HTML(), which Quarto/knitr renders as pure raw HTML with no
# markdown re-parsing. (An earlier draft used cat() under `results: asis`,
# which routes the output through pandoc's markdown parser -- that parser
# only recognizes certain patterns as raw HTML passthrough and silently
# HTML-escaped other fragments, e.g. the <option> tags. htmltools::HTML()
# sidesteps that entirely and is the standard approach for this.)
#
# Font/script tags are included here (inside the body output) rather than
# via Quarto's include-in-header, purely for simplicity -- browsers still
# load/apply <link>/<script src> tags correctly even when they're not
# literally inside <head>.

body_template<-r"---(<link rel="preconnect" href="https://fonts.googleapis.com">
...
final_body<-gsub("{{REPORT_TITLE}}", REPORT_TITLE, final_body, fixed = TRUE)
final_body<-gsub("{{YEAR_RANGE_LABEL}}", year_range_label, final_body, fixed = TRUE)
final_body<-gsub("{{OPTION_TAGS}}", option_tags, final_body, fixed = TRUE)
final_body<-gsub("{{GENERATED_DATE}}", generated_date_label, final_body, fixed = TRUE)
final_body<-gsub("{{REPORT_JSON}}", report_json, final_body, fixed = TRUE)

htmltools::HTML(final_body)
```
